...
const specialists_data = require('../data/specialists')
const patients_names = require('../data/patients')
const illness_data = require('../data/illnesses')
...
const Gameplay = () => {
    ...
    const loadPatientsData = () => {
        for (let i=0; i<200; i++) {
            let patient = {}
            patient.id = i
            patient.name = patients_names[Math.floor(Math.random() * patients_names.length)]
            patient.illness = illness_data[Math.floor(Math.random() * illness_data.length)]
            patient.age = Math.floor(Math.random() * (80 - 16 + 1) + 16)
            patient.health = Math.floor(Math.random() * (80 - 60 + 1) + 60)
    ...
    useEffect(() => {
        setSpecialists(specialists_data)
        }, [])  
    ...
    const handleAdmission = (event) => {
      ...
      const handleTreatment = (event) => {
        for (const patient of admittedPatients) {
            if (patient.id === parseInt(event.target.value)) {
                if (patient.specialist === patient.illness.specialist){
                    patient.health += 10
                } else {
                    patient.health -= 10
                }
    ...
    const handleDischarge = () => {
        for (const patient of patients) {    
            if (patient.health <= 0) {
                patient.status = "Done";
                setPoints(points - 50)
                setMessage(<p>{patient.name} died from {patient.illness.illness}</p>)
            ...
            if (patient.health >= 100) {
                ...
                setMessage(<p>{patient.name} got cured from {patient.illness.illness}</p>)
                setIsPatientPopupShown(true)
